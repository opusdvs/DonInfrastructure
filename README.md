...
#### 12.7. Автоматическое создание Pipeline (Job DSL)

Jenkins автоматически создаёт Pipeline jobs через JCasC и Job DSL plugin при установке.

**Автоматически создаваемые Pipeline:**

| Pipeline | Репозиторий | Описание |
|----------|-------------|----------|
| `donweather/ms-weather` | DonWeather-ms-weather | Multibranch Pipeline для микросервиса weather |
| `donweather/front` | DonWeather-front | Multibranch Pipeline для фронтенда |

**Проверка Pipeline:**

1. Откройте Jenkins: `https://jenkins.buildbyte.ru`
2. На главной странице должна быть папка **DonWeather**
3. Внутри папки — Pipeline jobs для каждого микросервиса
4. Multibranch Pipeline автоматически сканирует ветки и PR в репозиториях

**Конфигурация Pipeline:**

Pipeline определены в `helm/services/jenkins/jenkins-values.yaml` в секции `JCasC.configScripts.jobs-config`.

**Добавление нового Pipeline:**

```yaml
# В helm/services/jenkins/jenkins-values.yaml, секция JCasC.configScripts.jobs-config
- script: |
    multibranchPipelineJob('donweather/new-service') {
      displayName('new-service')
      description('CI/CD pipeline для нового микросервиса')
      branchSources {
        github {
          id('new-service-github')
          repoOwner('opusdvs')
          repository('DonWeather-new-service')
...
**Важно:**
- Pipeline используют credential `github-token` для доступа к репозиториям
- Репозитории должны содержать `Jenkinsfile` в корне для определения pipeline
- Multibranch Pipeline автоматически обнаруживает новые ветки каждые 5 минут
- Для применения изменений в Pipeline необходимо обновить Helm release Jenkins
